...
> 最后更新：2026-09-10（随心听与收藏复习采用页面级学习计时；TTS 安装机制保持不变）
...
- 学习统计新写入链路当前用于随心听和收藏复习：页面 → Provider → StudySessionTimer →
  StudyTimeService FIFO 队列 → StudyStatisticsDao 单事务；计时器按页面生命周期
  创建/销毁，播放完成事件写入输入统计，用户无活动超过 2 分钟才暂停总学习时长；其它学习
  任务在机制验证完成前保留原有 StudyEventRecorder / Stopwatch 写入链路，之后再逐项迁移。
